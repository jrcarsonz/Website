...
document.addEventListener('DOMContentLoaded', () => {
  const carousel = document.querySelector('.circle-wrapper');
  const cards = document.querySelectorAll('.service-item');

  if (!carousel || cards.length === 0) return;

  const cardWidth = cards[0].offsetWidth + 32; // card width + gap (2rem = 32px)
  const scrollPause = 2500; // time to pause before next scroll (ms)
  let currentIndex = 0;
  let timer;
  ...
  function startAutoScroll() {
    timer = setInterval(autoScroll, scrollPause);
  }
...
document.querySelectorAll('.flip-card').forEach(card => {
    ...
    // Optional: Allow keyboard accessibility (toggle flip on Enter or Space)
...
// Morph navbar from top to left on scroll
window.addEventListener('scroll', () => {
  const navbar = document.getElementById('morphic-navbar');
...
const images = document.querySelectorAll('.hero-image');
  ...
  function cycleImages() {
    ...
    setTimeout(() => {
      currentImg.classList.remove('active', 'zoom-out');
  ...
  window.addEventListener('load', () => {
    setTimeout(() => {
      document.querySelector('.s-intro__content-bg')?.classList.add('zoom-fade-out');
    }, 2000); // Delay in ms
  });
  ...
  function openModal(modalId) {
    ...
    if (modal) {
      modal.hidden = false;
      setTimeout(() => {
        modal.querySelector('.modal-popup').classList.add('show');
  ...
  // Optional: hook up to buttons or thumbnails
  // Example: document.querySelector('[data-modal="modal-01"]').addEventListener('click', () => openModal('modal-01'));
